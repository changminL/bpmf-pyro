...
class BPMF():
    ...
    def _model(self, ratings):
        ...
        while True:
            try:
                self.mu_item = pyro.sample('mu_item'
                  , dist.MultivariateNormal(i_mu_mean, covariance_matrix=i_mu_var))
                break
        ...
        while True:
            try:
                self.mu_user = pyro.sample('mu_user'
                , dist.MultivariateNormal(u_mu_mean, covariance_matrix=u_mu_var))
                break
            except (RuntimeError, ValueError):
        ...
        '''
        return pyro.sample('obs', dist.Normal(rmse, 0.5))

    def _conditioned_model(self,model, ratings):
        return poutine.condition(model, data={'obs' : torch.tensor(0.0,dtype = torch.float64)})(ratings)
 
    #1000, 1000, 1 for default
    def _main(self, ratings, jit=False, num_samples=10
                  , warmup_steps=4, num_chains=1):
        ratings = torch.autograd.Variable(torch.from_numpy(ratings))
        '''
        tm = poutine.trace(self._model(ratings)).get_trace()
        for name in tm.nodes:
            print("node : ")
            print(tm.nodes[name])
        '''
        ...
        posterior = MCMC(nuts_kernel,
                         ...
                         disable_progbar=False).run(self._model, ratings)
